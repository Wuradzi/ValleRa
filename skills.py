...
def translate_text(text, voice=None, listener=None):
    ...
    # Визначаємо мову
    target_lang = "uk"  # за замовчуванням - українська
    ...
    elif "на українську" in text or "на українську" in text:
        target_lang = "uk"
    ...
    # Видаляємо команди
    ignore = ["переклади", "перекладіть", "переклад", "переведи", "переведи"]
    for w in ignore:
    ...
    # Переклад через Google Translate (безкоштовно)
    try:
